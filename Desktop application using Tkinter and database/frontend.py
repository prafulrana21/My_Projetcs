"""
    A Programm that store this book information:
    Title, Author
    Year, ISBN
...
def get_selected_row(event): 
	"""
	This will give the data of the selectedd row
	"""
...
def update_command():
	...
	"""
    backend.update(selected_tuple[0],title_text.get(),Author_text.get(),Year_text.get(),isbn_text.get()) #update the selected row in the database
    list1.delete(0,END)
    for row in backend.view():
...
list1=Listbox(window,height=6,width=35) #create the listbox that show all the output of each command
...
list1.bind('<<ListboxSelect>>',get_selected_row) #this will help to collect the data of the selected row

b1=Button(window,text="View All", width=12 , command= view_command) #create the View All Button
b1.grid(row=2,column=3) #position the button

b2=Button(window,text="Search Entry", width=12, command=search_command)#create the Search Entry Button
b2.grid(row=3,column=3) #position the button

b3=Button(window,text="Add Entry", width=12,command=add_command)#create the Add Entry Button
